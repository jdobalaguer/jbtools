    ...
    for i_subject = 1:scan.running.subject.number
        for i_session = 1:scan.running.subject.session(i_subject)
            ...
            for i_model = 1:numel(scan.job.model)
                ...
                smooth_rs(:,:,:,i_model) = reshape(scan_nifti_load(file_rmap_image),scan.running.meta.dim);
                smooth_ps(:,:,:,i_model) = reshape(scan_nifti_load(file_pmap_image),scan.running.meta.dim);
            end
