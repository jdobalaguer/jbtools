...
function scan = scan_mvpa_run(scan)
    ...
    %% FUNCTION
    % assert
    assert(strcmp(scan.glm.function,'hrf'),'scan_mvpa_run: error. this only works with the canonical hrf function mode');
    ...
    mvpa_redo = true(1,1);
    if isfield(scan.mvpa,'redo'), mvpa_redo(1:scan.glm.redo-1) = false; end
    ...
    if do_mvpa_multivoxel,  scan = scan_mvpa_run(scan);             save_scan(); end    % MVPA:         run
