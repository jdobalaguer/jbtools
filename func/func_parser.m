...
function [optout,ret] = func_parser(optin,def)
    %% [optout,ret] = FUNC_PARSER(optin,def)
    % optin     - input options
    %             format :  cell of strings and values interleaved (e.g. varargin)
    %                       OR a struct with same format as [def]
    %                       OR 'parameters'
    %                       OR 'default'
    ...
    %% warnings
    error('func_parser: deprecated. use pair2struct() or struct_default() instead');
    
    %% function
    
    if ischar(optin), optin = {optin}; end
    % assert
