...
function auxiliar_design(varargin)
    ...
    if nargin~=2 || strcmp(varargin{2},'help')
        scan_tool_help(tcan,'@design(scan,subject)','This function opens the SPM Design Report (spm_DesRep) interface for the participant [subject]. You can use it to see the design matrix, the covariation between regressors');
        return;
    end
    SPM = file_loadvar(fullfile(tcan.running.directory.copy.first.spm,sprintf('subject_%03i',varargin{2}),'SPM.mat'),'SPM');
    % SPM = file_loadvar(fullfile(tcan.running.directory.copy.first.spm,sprintf('subject_%03i',tcan.running.subject.unique(varargin{2})),'SPM.mat'),'SPM');
