...
function scan = scan_glm_matrix(scan)
    ...
    for i_subject = 1:scan.running.subject.number
        ...
        % column
        scan.running.design(i_subject).column = struct('main',{{}},'name',{{}},'session',{[]},'order',{[]},'covariate',{[]},'number',{size(scan.running.design(i_subject).matrix,2)});
        j_column = 0;
        for i_session = 1:scan.running.subject.session(i_subject)
            % condition
            for i_condition = 1:length(scan.running.condition{i_subject}{i_session})
                for i_order = 1:n_order
                    j_column = j_column + 1;
                    scan.running.design(i_subject).column.main{j_column}      = scan.running.condition{i_subject}{i_session}(i_condition).name;
