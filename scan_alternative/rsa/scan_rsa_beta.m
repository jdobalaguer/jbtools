...
function scan = scan_rsa_beta(scan)
    ...
    for i_condition = 1:length(scan.job.glm.condition)
        ...
        scan.running.load.beta      = [scan.running.load.beta    ; single(scan.running.glm.function.get.beta  (condition,''))];
        ...
        scan.running.load.order     = [scan.running.load.order   ; single(scan.running.glm.function.get.vector(condition,'order'))];
