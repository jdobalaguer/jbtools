...
function scan = scan_glm_regressor_check(scan)
    ...
    for sub = scan.subject.u
        ...
        for i_run = u_run
            % load regressors
            file_datcon = sprintf('%scondition_sub_%02i_run_%02i.mat',scan.dire.glm.regressor,sub,i_run);
            file_datrea = sprintf('%srealign_sub_%02i_run_%02i.mat',  scan.dire.glm.regressor,sub,i_run);
