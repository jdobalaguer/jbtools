...
function handles = fig_barweb(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type)
    %%
    % Usage: handles = fig_barweb(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type)
    %
    % Ex: handles = fig_barweb(my_barvalues, my_errors, [], [], [], [], [], bone, [], bw_legend, 1, 'axis')
    %
    % fig_barweb is the m-by-n matrix of barvalues to be plotted.
    ...
    if nargin<4  || isempty(groupnames);    groupnames = 1:size(barvalues,1); end
    if nargin<5  || isempty(bw_title);      bw_title = [];                    end
    if nargin<6  || isempty(bw_xlabel);     bw_xlabel = [];                   end
    if nargin<7  || isempty(bw_ylabel);     bw_ylabel = [];                   end
    if nargin<8  || isempty(bw_colormap);   bw_colormap = fig_color('bw',size(barvalues,2))./255;   end
    if nargin<9  || isempty(gridstatus);    gridstatus = 'none';              end
    if nargin<10 || isempty(bw_legend);     bw_legend = [];                   end
    if nargin<11 || isempty(error_sides);   error_sides = 2;                  end
    ...
    assert(size(barvalues,1)==size(errors,1), 'barvalues and errors matrix must be of same dimension');
    assert(size(barvalues,2)==size(errors,2), 'barvalues and errors matrix must be of same dimension');
    ...
    if size(bw_colormap,1) == 1
        bw_colormap = repmat(bw_colormap,[size(barvalues,1),1]);
    end
    ...
    hold on;
    for i = 1:length(handles.bars)
        set(handles.bars(i),'facecolor',bw_colormap(i,:));
    end
