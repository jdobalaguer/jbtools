...
function scan = scan_tbte_condition_split(scan)
    ...
    for i_subject = 1:scan.running.subject.number
        ...
        for i_session = 1:length(scan.running.condition{i_subject})
            ...
            % variable
            condition = struct('main',{},'name',{},'onset',{},'subname',{},'level',{},'duration',{});
            ...
            for i_condition = 1:length(scan.running.condition{i_subject}{i_session})
                ...
                % split
                condition = [condition, ...
                             struct('main',     {scan.running.condition{i_subject}{i_session}(i_condition).name},...
                                    'name',     num2leg(1:n_condition,[scan.running.condition{i_subject}{i_session}(i_condition).name,'_%03i']),...
                                    'onset',    num2cell(mat2vec(scan.running.condition{i_subject}{i_session}(i_condition).onset)'),...
                                    'subname',  {{}},...
                                    'level',    {{}},...
