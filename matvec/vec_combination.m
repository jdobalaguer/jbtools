function m = vec_combination(varargin)
    %% m = VEC_COMBINATION(x1,x2,x3...)
    ...
    elseif NC > 1
        ...
        if any(isCellInput)
            ...
            m = cell(numel(ix{1}),NC) ; % pre-allocate the output
            for k=1:NC,
                % combine
                m(:,k) = reshape(varargin{k}(ix{k}),[],1) ;
            end
        else
            % non-cell input, assuming all numerical values or strings
            % flip using ii if last column is changing fastest
            [m{ii}] = ndgrid(varargin{ii}) ;
            % concatenate
            m = reshape(cat(NC+1,m{:}),[],NC) ;
        end
    elseif NC==1,
        m = varargin{1}(:) ; % nothing to combine
