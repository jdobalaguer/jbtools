...
function x = glm_design(s,m)
    ...
    for i_subject = 1:n_subject
        ...
        if m.zscoreX, x{i_subject} = nan2zero(mat_zscore(x{i_subject},1)); end % before interactions
        x{i_subject} = x2fx(x{i_subject},m.model);
        if m.zscoreX, x{i_subject} = nan2zero(mat_zscore(x{i_subject},1)); end % after interactions
    end
end
