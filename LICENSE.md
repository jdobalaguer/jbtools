
LICENSE

*********

The following code is referenced in this toolbox, but not included :

SPM12
    Wellcome Trust Centre for Neuroimaging (London, UK)

RSA
    Medical Research Council (Cambridge, UK)

GIT
    https://git-scm.com/

XJVIEW
    Xu Cui, Jian Li and Xiaowei Song
    http://www.alivelearn.net/xjview8/

PSYCHTOOLBOX

**********

The following code has been copied within this toolbox:

ANONYMOUS/CPRINTF.M
    Yair M. Altman - altmany@gmail.com

ANONYMOUS/DICT.M
    Doug Harriman - doug.harriman@gmail.com

ANONYMOUS/TREE.M
    Jean-Yves Tinevez - tinevez@pasteur.fr

D3/PRIVATE/WEBSERVER/JAVATCPSERVER.M,
     based on Dirk-Jan Kroon

FIGURES/FIG_4D.M
    Santiago Herce Castañón

FILE/FILE_2ABSOLUTE.M
    Jan Simon, Heidelberg - matlab.2013@n-simon.de

FUNC/FUNC_IS.M
    Jos van der Geest - jos@jasen.nl

FUNC/FUNC_WAIT.M
    Modification from "progress.m"
    Jeremy Scheff - jdscheff@gmail.com - http://www.jeremyscheff.com/

MTURK/MTURK_PARSEJSON.M
    Joel Feenstra

JB_TOOLS/JB_ANOVA.M
    based on repanova / myanova
    (Find the reference!)

MATVEC/VEC_COMBINATION.M
    Jos van der Geest - jos@jasen.nl

SCAN/RSA/SCAN_TOOL_RSA_SEARCHLIGHT.M
    Based on "fMRISearchlight.m"
    Cai Wingfield, Medical Research Council (Cambridge, UK)

SCAN/CONVERSION/PRIVATE/DICM2_NII.M
    Xiangrui Li

SCAN/SAVE/PRIVATE/SAVEFAST.M, SAVEFASTSTRUCT.M
    Timothy E. Holy

SCAN/SAVE/PRIVATE/HLP_SERIALIZE.M, HLP_DESERIALIZE.M
    Christian Kothe, Swartz Center for Computational Neuroscience, UCSD
    adapted from "SERIALIZE.M" and "DESERIALIZE.M", by Tim Hutt

SCAN/TOOL/SCAN_TOOL_RSA_VOXELSIZE.M
    by Xu Cui - http://www.alivelearn.net/?p=1101
