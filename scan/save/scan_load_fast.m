...
function lcan = scan_load_fast(scan,scan_directory)
    %% scan = SCAN_LOAD_FAST(scan)
    % load the [scan] struct, skipping files bigger than 1MB.
    ...
    for i_field = 1:length(u_field)
        ...
        try
            % variables
            if file_size(file{i_field}) > 1e6, continue; end
            var = u_field{i_field};
