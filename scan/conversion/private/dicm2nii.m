...
for i = 1:nRun
    ...
    if strncmpi(tryGetField(s, 'Manufacturer'), 'GE', 2)
        ...
        nSL = tryGetField(s, 'LocationsInAcquisition', nFile);
    ...
    elseif strncmpi(tryGetField(s, 'Manufacturer'), 'Philips', 7)
...
%     fprintf(fmtStr, fnames{i}, nFile); % show info and progress
    ...
    elseif ndims(img)==4 && tryGetField(s, 'Dim3IsVolume', false) % no-op now
...
if strncmpi(tryGetField(s, 'Manufacturer'), 'SIEMENS', 7) || isempty(name)
...
%     end
    ...
    % det from SliceNormalVector is not right: it will flip normal Sag slices.
    % The sPosition in asc_header correspond slices after following flip.
...
% Reorient if MRAcquisitionType==3D || slice_dim==3 || isDTI.
...
% try
%     s.nii_creator = ['dicm2nii.m ' datestr(now, 'yymmdd')];
%     fname = [tempname '.mat'];
...
if isfield(s, 'LastFile') && isfield(s.LastFile, 'ImagePositionPatient')
    ...
    if abs(pos-R(iSL,4)) < 1e-6, pos = []; end % the same slice, invalidate it
end
...
%% subfunction: return phase positive (in RAS system) and iPhase for xyz
function [phPos, iPhase] = phaseDirection(s)
R = reshape(s.ImageOrientationPatient, 3, 2);
[~, ixy] = max(abs(R));
iPhase = 2;
foo = tryGetField(s, 'InPlanePhaseEncodingDirection', ''); % not for Philips
