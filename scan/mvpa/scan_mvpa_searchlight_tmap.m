...
function scan = scan_mvpa_searchlight_tmap(scan)
    ...
    for i_regressor = 1:length(scan.mvpa.regressor.name)
        ...
        for i_subject = 1:scan.subject.n
            dmap_crispy(i_subject,:) = scan_nifti_load(sprintf('%sdmap_crispy_%s_subject_%d.img',scan.dire.mvpa.map,scan.mvpa.regressor.name{i_regressor},scan.subject.u(i_subject)));
            dmap_smooth(i_subject,:) = scan_nifti_load(sprintf('%sdmap_smooth_%s_subject_%d.img',scan.dire.mvpa.map,scan.mvpa.regressor.name{i_regressor},scan.subject.u(i_subject)));
        end
        
        % filenames
        file_dmap_crispy = sprintf('%sdmap_crispy_%s.img', scan.dire.mvpa.map,scan.mvpa.regressor.name{i_regressor});
        file_dmap_smooth = sprintf('%sdmap_smooth_%s.img', scan.dire.mvpa.map,scan.mvpa.regressor.name{i_regressor});
        file_tmap_crispy = sprintf('%stmap_crispy_%s.img', scan.dire.mvpa.map,scan.mvpa.regressor.name{i_regressor});
        file_tmap_smooth = sprintf('%stmap_smooth_%s.img', scan.dire.mvpa.map,scan.mvpa.regressor.name{i_regressor});
        file_pmap_crispy = sprintf('%spmap_crispy_%s.img', scan.dire.mvpa.map,scan.mvpa.regressor.name{i_regressor});
        file_pmap_smooth = sprintf('%spmap_smooth_%s.img', scan.dire.mvpa.map,scan.mvpa.regressor.name{i_regressor});
        file_hmap_crispy = sprintf('%shmap_crispy_%s.img', scan.dire.mvpa.map,scan.mvpa.regressor.name{i_regressor});
        file_hmap_smooth = sprintf('%shmap_smooth_%s.img', scan.dire.mvpa.map,scan.mvpa.regressor.name{i_regressor});
